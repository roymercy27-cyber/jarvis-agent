...
# Import Google instead of OpenAI
...
class Assistant(Agent):
    def __init__(self, chat_ctx=None) -> None:
        super().__init__(
            ...
            # Switched to Google Gemini Realtime
...
async def entrypoint(ctx: agents.JobContext):
    # Connect immediately to prevent timeout in Sandbox
    await ctx.connect()
    ...
    if results:
        ...
        initial_ctx.add_message(
            role="assistant",
            content=f"User: {user_name}. Context: {memory_str}."
        )
